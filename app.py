...
def parse_args():
    ...
    parser.add_argument("--length-scale", "--length_scale", type=float, help="Phoneme length (kecepatan suara)")
    parser.add_argument("--noise-scale", "--noise_scale", type=float, help="Generator noise")
    parser.add_argument("--noise-w-scale", "--noise_w_scale", type=float, help="Phoneme width noise")
    ...
    parser.add_argument("--data-dir", "--data_dir", action="append", default=[str(Path.cwd())])
    parser.add_argument("--download-dir", "--download_dir")
   
    parser.add_argument("--debug", action="store_true", help="Print DEBUG messages")
   
    parser.add_argument("--threads", type=int, default=8, help="Jumlah threads Waitress")
                ...
                with wave.open(wav_io, "wb") as wav_file:
                    ...
                    for i, chunk in enumerate(voice.synthesize(text, syn_config)):
                        ...
                        if i > 0:
                            ...
                            wav_file.writeframes(bytes([0] * silence_bytes))
